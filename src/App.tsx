...
function App() {
      ...
      fileReader.onload = (e) => {
        ...
        if (typeof result === 'string') {
          ...
          if (fields) {
            ...
            const uniqueFields = Array.from(new Set(fields.map((field: string) => field.replace(/[{}]/g, ''))));
  ...
  const handlePlayerWordsUpload = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files?.length === 1) {
      ...
      fileReader.onload = (e) => {
        ...
        const data: Array<any[]> = XLSX.utils.sheet_to_json(worksheet, { header: 1, blankrows: false, defval: null });

        // Skip the header row and start processing from the second row
        const newData = data.slice(1).reduce<{ [key: string]: string }>((acc, row) => {
  ...
  const generateSpreadsheet = () => {
    ...
    templateFields.forEach(field => {
      const id = field;
      const typeOfWord = field.split('-').slice(0, -1).join(' ').replace(/^\w/, c => c.toUpperCase());
  ...
  const canGenerateStory = templateFields.length > 0 &&
    ...
    <>
      ...
      <div className="upload-container">
        <div className="upload-box">
          ...
          <span className="info-icon" title="Use curly braces {} to indicate placeholders in the template. For example, {noun}, {verb}, {adjective}.">&#9432;</span>
          <input type="file" id="template-upload" onChange={handleTemplateUpload} />
        </div>
        <div className="upload-box">
          <label htmlFor="words-upload">Upload Player Words:</label>
          <input type="file" id="words-upload" onChange={handlePlayerWordsUpload} disabled={!canUploadPlayerWords} />
        </div>
      </div>
      <div className="card">
        <div className="generate-spreadsheet-container">
          <button onClick={generateSpreadsheet} disabled={!canGenerateSpreadsheet}>Generate Spreadsheet</button>
        </div>
      ...
      <div className="button-container">
        <button onClick={generateStory} disabled={!canGenerateStory}>Generate Story</button>
      </div>
